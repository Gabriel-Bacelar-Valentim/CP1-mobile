...
const GameScreen = ({ navigation }) => {
  ...
  const endGameText = hasWinner(table) ? `O jogador ${activePlayer} venceu!` : fullTable(table) ? 'Empate!' : '';
  ...
  const onCellClicked = (cellId) => {
    if (table[cellId] !== 0 || hasWinner(table) || fullTable(table)) {
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'space-around' }}>
      ...
      />
      <InvalidMoveDialog isOpen={invalidMoveDialog} onClickClose={() => setInvalidMoveDialog(false)} />
    </View>
  );
}
